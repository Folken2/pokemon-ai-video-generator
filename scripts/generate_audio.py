...
def generate_audio(text, output_path, api_key, voice_id,
                  model_id="eleven_multilingual_v2",
                  stability=0.40, similarity_boost=0.75, style=0.12):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate narrator audio using ElevenLabs v3'
    )
    parser.add_argument(
        '--text',
        required=True,
    ...
    parser.add_argument(
        '--output',
        required=True,
        help='Output file path (e.g., pikachu/audio/clip_01.mp3)'
    )
    parser.add_argument(
        '--model',
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.40,
        help='Voice stability 0-1 (default: 0.40 for 40%%)'
    )
    parser.add_argument(
        '--similarity',
        type=float,
